shaman_mystic_snake = class({})

LinkLuaModifier( "modifier_shaman_mystic_snake", "lua_abilities/shaman_mystic_snake/modifier_shaman_mystic_snake", LUA_MODIFIER_MOTION_NONE )

function shaman_mystic_snake:OnSpellStart()
...
shaman_mystic_snake.projectiles = {}
function shaman_mystic_snake:OnProjectileHit_ExtraData( target, location, ExtraData )
	-- load data
	...
	if target and (not target:IsMagicImmune()) and (not target:IsInvulnerable()) then
		...
		data.health_stolen = data.health_stolen + ( health - target:GetHealth() )
...
function shaman_mystic_snake:Returning( data, target )
	if not target then
		self:Returned( data )
		return
	end
...
function shaman_mystic_snake:Returned( data )
	...
	self:GetCaster():Heal( data.health_stolen/2, self )
	...
	local block = data.health_stolen/2/#allies

	for _, ally in pairs(allies) do
		ally:AddNewModifier(
		self:GetCaster(),
		self,
		"modifier_shaman_mystic_snake",
		{	
			duration = 8,
			block =  block }
		)
...
shaman_mystic_snake.unique = {}
shaman_mystic_snake.i = 0
shaman_mystic_snake.max = 65536
function shaman_mystic_snake:GetUniqueInt()
...
function shaman_mystic_snake:DelUniqueInt( i )
	self.unique[ i ] = nil
end