...
function furion_sprout_lua:OnSpellStart()
	...
	local tree_model = "models/items/tiny_01/tiny_haunted_tree.vmdl"
	...
	if hTarget == nil or ( hTarget ~= nil and ( not hTarget:TriggerSpellAbsorb( self ) ) ) then
		...
		for i = 1,8 do
			CreateTempTreeWithModel( vTargetPosition + Vector( x_offset[i], y_offset[i], 0.0 ), self.duration, tree_model)
		end
