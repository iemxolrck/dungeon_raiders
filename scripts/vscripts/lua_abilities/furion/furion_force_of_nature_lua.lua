...
LinkLuaModifier( "modifier_furion_force_of_nature_lua", "lua_abilities/furion/modifier_furion_force_of_nature_lua",LUA_MODIFIER_MOTION_NONE )
...
function furion_force_of_nature_lua:OnSpellStart()
	self.birds = {"npc_dota_bird", "npc_dota_hawk", "npc_dota_eagle", "npc_dota_parrot", "npc_dota_raven", "npc_dota_nightowl", "npc_dota_falcon", "npc_dota_phoenix_0"}
	...
	while nTreantsToSpawn > 0 do
		local hTreant = CreateUnitByName( "npc_dota_druid_treant", vTargetPosition, true, self:GetCaster(), self:GetCaster():GetOwner(), self:GetCaster():GetTeamNumber() )
		if hTreant ~= nil then
